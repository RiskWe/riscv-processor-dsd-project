...
module leading_zero_counter_sync (
    ...
    integer i;
    reg found; // Flag to indicate the first '1' has been found

    always @(posedge clk) begin
        count = 0;
        found = 0; // Reset the flag at the start of each clock cycle

        // Loop through all 24 bits
        for (i = 23; i >= 0; i = i - 1) begin
            if (!found && in[i] == 1'b1) begin
                count = 23 - i;
                found = 1; // Mark that we've found the first '1'
            end
        end
    end
endmodule
